...
type Hook interface {
	...
	Special(string, *tunnel.Metadata) (net.Conn, error)
}
...
func (s *Server) Special(password string, metadata *tunnel.Metadata, c net.Conn) {
	defer c.Close()
	rc, err := s.hook.Special(password, metadata)
	if err != nil {
		return
	}
	defer rc.Close()
	...
	case ch := <-errChan:
		if ch != nil {
			s.log.Errorf("special forward error %v", ch.Error())
		}
	case <-s.ctx.Done():
		s.log.Debug("special forward closed")
		return
	}
}
